...
orderTake(4000, () => {}).then(async () => {
  rl.question(
    ...
    (answer) => {
      if (answer == "1") {
        //Köfte seçildiğinde çalışan kod bloğu
        console.log("İstenilen: Köfte");

        return orderTake(1000, () => {}).then(async () => {
          //Köftenin pişme derecesi Degree() ile sağlandı.
          rl.question(
            "Köftenizi nasıl istersiniz? 1-Az pişmiş / 2-Orta pişmiş / 3-Çok Pişmiş\n",
            (message) => Degree(message)
          );

          return orderTake(5000, () => {}).then(async () => {
            //Ektra eklenecek olan bileşenler Extra() ile sağlandı.
            rl.question(
        ...
        //Tavuk seçildiğinde çalışan kod bloğu
